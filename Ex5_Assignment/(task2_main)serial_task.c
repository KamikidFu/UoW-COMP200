...
void main(){
	...
	while(1){
		...
		if(statusReceiveLetter == '1'){
			...
			//Other illeage letter, just continue to next loop
			continue;
		}
		...
		}else if(status == 2){	//Status 2, "\rssss.ss"
			//Using loop to check status, and transmit '\r', to SP2
			//To make it return at beginning of line
			while(1){
				if((*sp2_status & 2)!=0){
		...
		}else if(status == 3){	//Status 3, "\rtttttt"
			//Using loop to check status, and transmit '\r', to SP2
			//To make it return at beginning of line
			while(1){
				if((*sp2_status & 2)!=0){
		...
		}else if(status == 0)	//Status 0, quit application
